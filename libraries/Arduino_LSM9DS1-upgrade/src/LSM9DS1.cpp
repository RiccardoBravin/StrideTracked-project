...
{ int16_t data[3];
  ...
  float scale = getAccelFS()/32768.0 ; //correctly implemented to transform to float but lowers precision   
  //float scale = 1;  //modified line to get back int16_t data type inside a float
...
//FUNCTION ADDED FOR FASTER TRANSFER
int LSM9DS1Class::readRawAccelArr(int16_t data_out[3])   // return raw uncalibrated data 
{ 
  int16_t data[3];
  if (!readRegisters(LSM9DS1_ADDRESS, LSM9DS1_OUT_X_XL, (uint8_t*)data, sizeof(data))) {  
    //data = 0;   
  ...
  memcpy(data_out, data, 3*sizeof(int16_t));
  return 1;
}
...
int LSM9DS1Class::readRawGyroArr(int16_t data_out[3])   // return raw data for calibration purposes
{ 
  int16_t data[3];
  if (!readRegisters(LSM9DS1_ADDRESS, LSM9DS1_OUT_X_G, (uint8_t*)data, sizeof(data))) { 
    return 0;
